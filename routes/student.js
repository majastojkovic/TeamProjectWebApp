...
router.post('/leaveTeam', ensureAuthenticatedStudent, (req, res, next) => {
  ...
  Student.findOneAndUpdate({ email: req.user.email }, { teamName: null }, function(err, student) {
    ...
    Team.findOne({ name: student.teamName }, function(err, team) {
      ...
      if (newNumberOfMembers == 1) {
        ...
        Student.findOneAndUpdate( { _id: teamMembers[0] }, { teamName: null }, function(err, student) { });
...
router.get('/createTeam', ensureAuthenticatedStudent, (req, res) => res.render('createTeam'));
